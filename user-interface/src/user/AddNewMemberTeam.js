...
function AddNewMemberTeam() {
    ...
    const [showingLast, setshowingLast] = useState(1);
    const [departmentOptions, setdepartmentOptions] = useState([]);
    const [dropdownName, setDropdownName] = useState("All department");
    ...
    const reviewYourTeam = (e) =>{
        let AddNewMemberTeam = localStorage.getItem("ManageTeamMember")
        if(AddNewMemberTeam == null || AddNewMemberTeam =="" ){
            alert("Please select your team member")
        }else{

            let team_type = firstData[0].team_type
            let estimated_monthly_cost 
            let sla_support_cost = firstData[0].sla_support_cost
            let team_member = localStorage.getItem("ManageTeamMember")
            let requisition_id = firstData[0].requisition_id

            let total_cost_by_talent  = EstimatedTotalCost

            if(team_type == 1){
                estimated_monthly_cost = EstimatedTotalCost
            }else{
                estimated_monthly_cost = EstimatedTotalCost * .2 + EstimatedTotalCost
                sla_support_cost = EstimatedTotalCost * .2
            }

            axios.post('AddNewTeamMember', {
                TeamId: TeamId.id,
                team_type,
                estimated_monthly_cost,
                sla_support_cost,
                team_member,
                total_cost_by_talent,
                requisition_id
            })
            .then(function (response) {
        
                routerHistory.push("/db_manage_team/"+TeamId.id);
    ...
    const Calculator = ()=>{
        let total = 0;
        for (let index = 0; index < data.length; index++) {
            if( localStorage.getItem("ManageTeamMember").includes(data[index].id) ){
    ...
    const RemoveTeamMemberAlreadyExist =(e, id , key) =>{
        let MemberID =  localStorage.getItem('ManageTeamMember');

        let array = [];
        for( var i = 0; i < MemberID.length; i++){ 

            if ( MemberID[i] != id && MemberID[i] !="," ) { 
                array.push(MemberID[i])
            }
        }

        localStorage.setItem('ManageTeamMember' , array )

        let slideItem = "slider-item-" + key;
        ...
        let slideAddItem = "add-team-"+key
        if(document.getElementById(slideAddItem) != undefined ){
    ...
    const slickSlider = () =>{
        let xxxxxx = document.getElementsByClassName("slider-item")
        for (let index = 0; index < xxxxxx.length; index++) {
            xxxxxx[index].style.display = "block";
        }
    }

    const AddTeam = (e , id , member_id) => {
        ...
        if(TeamMemberKey.includes(id)){
            ...
            let teammembers = localStorage.getItem("ManageTeamMember").split(",")
            let arr
            arr = teammembers.filter(function(item) {
                return item != id
            })

            // for (let index = 0; index < teammembers.length; index++) {
            //     if(teammembers[index] != ",")array.push( parseInt(teammembers[index])  ) 
            // }

            arr.push(member_id);
            localStorage.setItem('ManageTeamMember', arr )
    ...
    function fetchData_first(){

        var AuthUserToken = localStorage.getItem("AuthUserToken")
        axios.defaults.headers.common['AuthUserToken'] = AuthUserToken;
    
        axios.post('getTeamMember', {
          TeamId: TeamId.id
          })
          .then(function (response) {
    
            if (response.data.length == 0 ) {
              cogoToast.success("Team has been removed" , { position: 'top-right' } );
              routerHistory.push("/");
            } else{
              setFirstData(response.data)
              let teamMember = []
              for (let index = 0; index < response.data.length; index++) {
                teamMember.push(response.data[index].employee_id)
                if(response.data[index].team_type ==2 ){ 
                  localStorage.setItem("ManageTeamType" , "sla" )
                }else{
                  localStorage.setItem("ManageTeamType" , "offshore" )
                }
              }
              
              localStorage.setItem("ManageTeamMember" , teamMember )
      ...
      function fetchData_2(filter="" , status="" , dropdownSort=""){
        ...
        .then(res => {
            let total = 0;
            let TeamMember = []
            if(localStorage.getItem("ManageTeamMember") != null && localStorage.getItem("TeamMemberID") != ""){ 

                for (let index = 0; index < res.data.length; index++) {
                    
                    if( localStorage.getItem("ManageTeamMember").split(",").includes( res.data[index].id.toString() )  ){
                        total += res.data[index].salary;
                        localStorage.setItem('ManageTeamMember', localStorage.getItem("ManageTeamMember") );

                        TeamMember.push( 
                            <div className={ "slider-item slider-item-" + index } key={index} >
                                <img src={ res.data[index].emp_image ? S3KEY+ res.data[index].emp_image : UserPic} alt="User Pic" />
                                <h3> { res.data[index].emp_name } </h3>
                                <p> { res.data[index].depertment } </p>
                                <div className="slider-overlay" onClick={ (e) => { RemoveTeamMemberAlreadyExist(e, res.data[index].id , index) } } >
                                    <svg
                                        width={20}
                                        height={20}
                                        viewBox="0 0 20 20"
    ...
    function paginateClick(e) {
        ...
        setshowingStart(showingStart)
        setshowingLast(showingLast)
    }
    ...
    function sortDropdown(params , e , dept_name) {
        setRefresh(!refresh)
        document.getElementById("dropdown-basic").innerHTML = dept_name 
        fetchData("" , "" , params )
        setDropdownName(dept_name)
    }
        ...
        <div className="right_content">
            <div className="your-team-name">
                ...
                <div className="btn_manage">
                    ...
                    <Link onClick={reviewYourTeam} to="/db_manage_team" > 
                    Submit Requisition
                        <svg
                            width={11}
                            height={10}
                    ...
                    <div className="btn_manage back_R" >


                    <Link onClick={backToTeam} to="#"> 
                    Back 
                        <svg
                            width={11}
                            height={10}
                    ...
                    if (TeamMember.length == 0 ) {
                        ...
                        return(
                            <div className="no_member_box-item">
                                ...
                                {(() => {
                                    if (TeamMember.length > 5) {
                                        return(
                                            <Slider {...settings_infinite} dots={false}>
                                                {TeamMember}
                                            </Slider>
                                        )
                    ...
                    <div className="depart-ul">
                        <ul className="Sort_by_flex">
                            ...
                            <Dropdown>
                                ...
                                {dropdownName} <svg width="13" height="8" viewBox="0 0 13 8" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <Dropdown.Menu>
                                        <Dropdown.Item onClick={ (e)=>sortDropdown("", e , "All Department" ) } style={{ cursor:"pointer" }}> All Department </Dropdown.Item>
                                    {
                                        departmentOptions.map((data , key)=>
                                        <Dropdown.Item key={key} onClick={ (e)=>sortDropdown(data.id, e , data.name ) } style={{ cursor:"pointer" }}>{data.name}</Dropdown.Item>
                                        )
                                    }
                                </Dropdown.Menu>
                            </Dropdown>
                    ...
                    <div className="row">
                        <div className="col-md-12">
                            <div className="r_number_u">
                                ...
                                <p id="showingInfo" >Showing { showingStart }- {showingLast} of {data.length} </p>
                                </div>
                                <nav
                            ...
                            <div className= {key <10 || key ==0 ? "col-lg-6 pl-0 pr_padding" : "col-lg-6 pl-0 pr_padding display_none" } key={key} id={"employee_" + key} data-name={post.emp_name} >
                                <div className= {post.emp_status == 1 ? "department-profile" : "department-profile engaged" } >
                                    ...
                                    <div className="resume-part-flex">
                                        ...
                                        {(() => {
                                            if ( post.emp_status == 1 ) {

                                                return(

                                                <div className={ localStorage.getItem("ManageTeamMember").split(",").includes( post.id.toString() ) ? "display_none" : "add-team" }   onClick={(e)=> AddTeam( e ,key , post.id  ) } id={"add-team-"+ key } >
                                        ...
                                        {(() => {
                                            if ( post.emp_status == 0 ) {
                                                return(
                                                    
                                                    <div className="add-team disabled"  >
                                                        <a style={{ cursor: "pointer" }} >
                                                        <svg
                                                            width={13}
                                                            height={13}
                                        ...
                                        {(() => {
                                            if ( localStorage.getItem("ManageTeamMember") ) {
                                                if( localStorage.getItem("ManageTeamMember").split(",").includes( post.id.toString() )   ){

                                                    return(
                                                    
                                                        <div className="removeTeam" onClick={ (e)=> RemoveTeamMemberAlreadyExist(e, post.id , key) } style={{ cursor:"pointer" }} >
            ...
            <div className="row user_resume">
                ...
                <div className="row">
                    ...
                    <h2 className="resume_userName"> {Modaldata.emp_name ? Modaldata.emp_name : "" } </h2>
                        ...
                        <button type="button" className="btn btn-downloadResume" onClick={(e)=> downLoadPdf(e, Modaldata.emp_name)} ><svg width={16} height={20} viewBox="0 0 16 20" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export default AddNewMemberTeam;