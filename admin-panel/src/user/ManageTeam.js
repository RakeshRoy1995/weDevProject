...
import { S3KEY , FrontURL } from '../Constant';
...
function ReviewYourTeam() {
  ...
  const RemoveMember =(e, id , key) =>{
    ...
    let deleteAllmember = false
    if(firstData.length == 1){
      deleteAllmember = true
    }

    let type = localStorage.getItem("ManageTeamType")
    let employeeSalary
    let employeeTotalSalary 
    let total_monthly_cost
    let sla_support_cost = 0
    let name = localStorage.getItem("ManageTeamName")
    let total_team_member = firstData.length 

    for (let index = 0; index < firstData.length; index++) {
      if(firstData[index].employee_id == id ){
        employeeSalary = firstData[index].salary
        employeeTotalSalary = firstData[index].total_cost_by_talent
      }
    }

    let total_cost_by_talent = employeeTotalSalary - employeeSalary

    if(localStorage.getItem("ManageTeamType") =="sla" ){
      total_monthly_cost = total_cost_by_talent * .2 + total_cost_by_talent
      sla_support_cost = total_cost_by_talent * .2 
    }else{
      total_monthly_cost = total_cost_by_talent;
    }
    let ManageTeamMember = localStorage.getItem("ManageTeamMember")


    localStorage.setItem("typeType", type)
    localStorage.setItem("teamName", name )
    localStorage.setItem("TeamMemberID", ManageTeamMember)
    localStorage.setItem("teamCost", total_cost_by_talent)
    localStorage.setItem("slaCost", sla_support_cost)
    localStorage.setItem("editTeamID", requisition_id)

    window.location.href = FrontURL + "review_your_team"
  ...
  //     type,
  //     teamId : TeamId.id,
  //     requisition_id: firstData[0].requisition_id
  //     })
  //     .then(function (response) {
  //       fetchData()

  //       cogoToast.success("Team Member Removed" , { position: 'top-right' } );
  //     }).catch(function (error) {
  ...
  const teamType = async(e , type)=>{
    let totalEstimate = 0
    let slaCost = 0
    let total_cost_by_talent = firstData[0].total_cost_by_talent
    let teamType = 1

    if(type == "sla"){
      teamType = 2
      slaCost = total_cost_by_talent * .2;
      totalEstimate = total_cost_by_talent * .2 + total_cost_by_talent
    }else{
      totalEstimate = total_cost_by_talent * 0 + total_cost_by_talent
    }

    axios.post('updateTeamCost', {
        TeamId: TeamId.id,
        type: teamType,
        estimated_monthly_cost: totalEstimate,
        slaCost,
        total_cost_by_talent,
        requisition_id: firstData[0].requisition_id
    })
    ...
    axios.post('getTeamMember', {
      ...
      .then(function (response) {
        ...
        if (response.data.length == 0 ) {
          ...
          localStorage.removeItem("ManageTeamType")
          localStorage.removeItem("ManageTeamName")
          localStorage.removeItem("ManageTeamMember")
          routerHistory.push("/");
        } else{
          ...
          for (let index = 0; index < response.data.length; index++) {
            ...
            localStorage.setItem("ManageTeamName" , response.data[index].name )
          }
          localStorage.setItem("ManageTeamMember" , teamMember )

          

          setEstimatedTotalCost( response.data[0].sla_support_cost )
          setteamMemberCost(response.data[0].total_monthly_cost )
          setrequisition_id(response.data[0].requisition_id )
  ...
  function fetchData_Main(){

    var AuthUserToken = localStorage.getItem("AuthUserToken")
    axios.defaults.headers.common['AuthUserToken'] = AuthUserToken;

    axios.post('reviewYourTeam', {
      teamMembers: localStorage.getItem("ManageTeamMember")
      })
      .then(function (response) {
        ...
        if(response.data.length !=0){
          ...
          let TeamMemberID = localStorage.getItem("ManageTeamMember")
          ...
          document.getElementById("takeTeamName").innerHTML = localStorage.getItem("ManageTeamName")
        ...
        <div className="your-team-name">
            ...
            <div className="row">
              <div className="col-lg-5">
                <div className="Cal_cost_overlay_text">
                  <h3>
                    Total monthly cost <span>${ teamMemberCost } </span>
                  </h3>
              ...
              <div className="col-lg-5">
                <div className="Cal_cost_overlay_text">
                  <h3>
                    SLA Support <span>${ Number(EstimatedTotalCost).toFixed(5) }</span>
                  </h3>
          ...
          <div className="teamHire">
            ...
            <div className="teamHier_top">
              <h1>  { firstData.length != 0 ? firstData[0].name : "" } </h1>
              <button type="button" name="meetin_schedule" className={firstData.length != 0 && firstData[0].status != 1 ? "btn btn-meetin_pending" : "btn btn-meetin_schedule" }  > { firstData[0]?.status == 0 ? "Meeting Pending" : <> { firstData[0]?.status == 1 ? "Meeting Scheduled" : <> { firstData[0]?.status == 2 ? "Searching" : <> { firstData[0]?.status == 3 ? "Hiring Completed" : "Cancelled" } </> } </>  } </> }   </button>
            </div>
              ...
              <div id="tabs">
                <div className="teamTypeSelect">
                  <ul>
                    ...
                    <li onClick={(e)=> teamType(e , "offshore") } id="Offshore" className={firstData.length != 0 &&  firstData[0].team_type == 1  ? "ui-state-active" : " " } >
                      <a className="Offshore">
                        <img src={OffshoreImg} alt="images" className="Offshore"  /> Offshore
                        <br /> Dedicated Team
                      </a>
                    </li>
                    <li onClick={(e)=> teamType(e , "sla") } id="SLA" className={firstData.length != 0 &&  firstData[0].team_type == 2  ? "ui-state-active" : " " }  >
                      <a className="SLA">
                ...
                <div className="teamSupporting">
                  ...
                  <div id="tabs-3" style={firstData.length != 0 &&  firstData[0].team_type == 1 ? { display: "none" } : { display: "block" } } >
                  ...
                  <div id="tabs-2" style={firstData.length != 0 &&  firstData[0].team_type == 2  ? { display: "none" } : { display: "block" } }  >
          ...
          <div className="hireTeam_list">
            ...
            <div className="db_add_Member">
              ...
              <Link to="#" onClick={ (e)=> RemoveMember(e,data[0].id , 0) } style={{ cursor:"pointer" }} className="add_member_a" >
                <svg
                  width={13}
                  height={13}
                  viewBox="0 0 13 13"
              ...
              {/* <Link to={ firstData.length != 0 ? "/add_new_member_team/" + firstData[0].id : "" }  className="add_member_a" >
                <svg
                  width={13}
                  height={13}
                  viewBox="0 0 13 13"
